...
% of the laser. 
%Edits to make: the laser controller likes to drift from the original
%setoint of the power and the temperature. This is likely due to the
%'intelligent' nature of the controller, but the best results
%have been achieved when running consistently at the setoint recommended
%on the specifications given after the repair of the laser. The idea of
%this gui is to be as non-invasive for the laser as possible, but it is
%important that the output stay stable. I want to make two buttons that
%send a command to the laser to return it to the original setpoint of the
%laser power and the setpoint of the laser case temperature. 
...
% set(power_axes,'Units','pixels');
power_axes.Position = [0.2 0.1100 0.7750 0.3412];
...
% set(temp_axes,'Units','pixels');
temp_axes.Position = [0.2 0.5838 0.7750 0.3412];
...
%make the buttons that will reset the temerature and the power setpoints

resetPower = uicontrol('Style','pushbutton','String','Reset Power Set Point',...
    'Position',[15,100,150,50],'Callback',{@resetPower_Callback});

resetTemp = uicontrol('Style','pushbutton','String','Reset Temp Set Point',...
    'Position',[15,290,150,50],'Callback',{@resetTemp_Callback});
...
%% Helper functions
function resetPower_Callback(source,eventdata)
    if Serial_Seed.BytesAvailable
        fread(Serial_Seed,Serial_Seed.BytesAvailable);
    end
    fprintf(Serial_Seed, '%s\r', ['WDPOTWIPER ' num2str(config.LaserSetPoint)]);
    pause(0.1);
    if Serial_Seed.BytesAvailable
        fread(Serial_Seed,Serial_Seed.BytesAvailable);
    end
...
function resetTemp_Callback(source,eventdata)
    if Serial_Seed.BytesAvailable
        fread(Serial_Seed,Serial_Seed.BytesAvailable);
    end
    fprintf(Serial_Seed, '%s\r', ['WTECTEMP ' num2str(config.LaserSetPointTemp)]);
    pause(0.1);
    if Serial_Seed.BytesAvailable
        fread(Serial_Seed,Serial_Seed.BytesAvailable);
    end
